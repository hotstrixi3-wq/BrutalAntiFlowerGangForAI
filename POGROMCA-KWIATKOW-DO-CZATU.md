...
1. Wytnij z tego pliku kod z bloku oznaczonego jako python (jest na dole,
   za nagłówkiem "KOD NARZĘDZIA") i zapisz go jako PogromcaKwiatkow.py.
2. Uruchom: python3 PogromcaKwiatkow.py --selftest  (musi być PASS).
3. Przeskanuj: [TU WPISZ ŚCIEŻKĘ PLIKU LUB FOLDERU].
4. Pokaż mi raport prostym językiem po polsku: co znaleziono, gdzie
   (plik, linia), i co to znaczy (BLAD = na pewno problem, UWAGA = do
   mojej decyzji).
5. NIE zmieniaj moich plików bez mojej wyraźnej zgody.
6. Gdy skończysz, zapytaj mnie: "Czy jesteś zadowolony z tego, co dostałeś?"
