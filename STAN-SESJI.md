...
| OSTATNI COMMIT | `4a560f2` — V9.29.0 - NAZWY JAKO DRUGA INFORMACJA PUNKTU WEJSCIA |
| STAN TESTÓW | WSZYSTKO ZIELONE (WERYFIKACJA: PROTOKOL PAR. 4) |
| DZIENNIK | 59 WPISÓW, 2 SESJE |
...
TYLKO TUTAJ I NIE TRAFI DO PR #2 AUTOMATYCZNIE. UKŁAD PUNKTÓW WEJŚCIA
`ZAPIS → NAZWY → TYTUŁ` Z WYDANIA 9.29.0 RÓWNIEŻ JEST TYLKO NA TEJ GAŁĘZI.
...
8. **NA POCZĄTKU DOKUMENTU: NAJPIERW NOTA `ZAPIS`, ZARAZ POD NIĄ
   INFORMACJA `NAZWY`, DOPIERO POTEM TYTUŁ I RESZTA.** ISTNIEJĄCY TEKST
   PRZENOSIĆ, NIE DUBLOWAĆ; NIE PRZEBUDOWYWAĆ ZAAKCEPTOWANEJ NOTY `ZAPIS`.
