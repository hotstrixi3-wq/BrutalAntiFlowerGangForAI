...
### ZMIANA FORMATU DOKUMENTU MOŻE ZERWAĆ KOD, KTÓRY GO AKTUALIZUJE

ZMIERZONY PRZYPADEK: `STAN-SESJI.md` ZMIENIONO NA WIELKIE LITERY, ALE
`pamietnik.py --stan` NADAL SZUKAŁ CZTERECH MAŁYCH ETYKIET. KAŻDY REGEX
ZWRÓCIŁ ZERO TRAFIEŃ. NARZĘDZIE MIMO TO WYPISAŁO `[OK]`, POKAZAŁO
`(0 pol)` I ZWRÓCIŁO KOD 0, A STAN UTKNĄŁ NA WERSJI 9.19.0 PRZY
FAKTYCZNEJ 9.28.0.

AKTUALIZATOR DOKUMENTU MUSI NAJPIERW SPRAWDZIĆ CAŁY KONTRAKT STRUKTURY:
KAŻDE OCZEKIWANE POLE MA WYSTĄPIĆ **DOKŁADNIE RAZ**. DOPIERO POTEM BUDUJE
CAŁY WYNIK W PAMIĘCI I ZAPISUJE GO ATOMOWO. ZERO, DUPLIKAT ALBO BRAK
DANYCH → `exit 2` I PLIK BAJT W BAJT NIETKNIĘTY.

TEST REGRESYJNY MUSI UŻYWAĆ REALNEGO FORMATU DOKUMENTU — TU WARIANTU
Z WIELKIMI ETYKIETAMI — ORAZ OSOBNO SPRAWDZAĆ ODMOWĘ NA NIEPEŁNEJ TABELI.
